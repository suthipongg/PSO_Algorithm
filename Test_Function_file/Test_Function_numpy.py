...
class func_2D:
    def __init__(self):
        self.bound_Ackley = [-5,5]
        self.bound_Levi_N13 = [-10,10]
        self.bound_Eggholder = [-512,512]
        self.bound_Holder_table = [-10,10]
        self.bound_Rastrigin = [-5.12,5.12]
        self.bound_Schwefel = [-500,500] 
        self.n_func = self.all_func().shape[0]

    def Ackley(self, x):
        return -20.0 * exp(-0.2*sqrt(0.5*(x[:, 0]**2 + x[:, 1]**2))) - exp(0.5*(cos(2*pi*x[:, 0]) + cos(2*pi*x[:, 1]))) + exp(1) + 20
